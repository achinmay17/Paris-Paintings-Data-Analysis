...
#all the plots
p2<- ggplot (data= train, aes (y = price, x = fig_mention, col = fig_mention))+
  geom_boxplot() +
  scale_y_log10()
...
p4 <- ggplot (data= train, aes (y = price, x =  artist_living_notliving, col = artist_living_notliving))+
  geom_boxplot() +
  scale_y_log10()
...
p7<-ggplot (data= train, aes (y = price, x =  finished_nofinished, col = finished_nofinished))+
  geom_boxplot() +
  scale_y_log10()

p8<-ggplot (data= train, aes (y = price, x = mytho_nomytho , col = mytho_nomytho ))+
  geom_boxplot() +
  scale_y_log10()

p9<-ggplot (data= train, aes (y = price, x =  history_nohistory, col = history_nohistory))+
  geom_boxplot() +
  scale_y_log10()

p10 <- ggplot (data= train, aes (y = price, x =  LF, col = LF))+
  geom_boxplot() +
  scale_y_log10()
```
...
```{r,echo=FALSE}
#boxplots
multiplot(p2,p3, p4,p7,p8,p9,p10, cols=2)
```
...
According to our summary, we believe that interactions are important. In our model, the most important variables are "dealer" and "winningbiddertype", which have been explained above. 