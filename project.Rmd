...
#Create new variable "wb"
wb_list <- train %>%
  group_by(winningbiddertype) %>%
  summarise(mean_price = mean(price)) %>%
  ungroup() %>%
  arrange(desc(mean_price))
train <- train %>%
  mutate(wb = ifelse(winningbiddertype %in% 
                       wb_list$winningbiddertype[which(wb_list$mean_price >= 1000)],1,0))


#create new variable "buyer_bino"
buyer_list <- train %>%
  group_by(endbuyer) %>%
  summarise(mean_price = mean(price)) %>%
  ungroup() %>%
  arrange(desc(mean_price))
train <- train %>%
  mutate(buyer_bino = ifelse(endbuyer %in% 
                       buyer_list$endbuyer[which(buyer_list$mean_price >= 1000)],1,0))
...
#winningbiddertype removed due to potential collinearity with wb
m <- lm(formula = log(price) ~ dealer + year + school_pntg + diff_origin + 
        ...
        Surface:mat_recode + famous_author:Surface + wb:Surface + buyer_bino:Surface, data = newTrain)
