...
m2 <- gbm(log(price) ~ dealer + year + school_pntg + diff_origin + 
        artistliving + winningbiddertype + Surface + engraved + 
        prevcoll + paired + finished + lrgfont + othgenre +  
        discauth + mat_recode + famous_author, data = newTrain,
...
predictions1 = as.data.frame(
  exp(predict(m1, newdata=newTest, 
              interval = "predict",type="link",n.trees=20000)))

predictions <- cbind(predictions1, predictions)
predictions <- predictions[-c(2)]
