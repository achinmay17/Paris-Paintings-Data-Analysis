...
#        discauth+ winningbiddertype:prevcoll + 
#        Surface:mat_recode + famous_author:Surface, data = newTrain,mtry=5,
#        ntree=15000) #confrimed at 6:35 add back interaction.
...
```{r}
newTest <- newTest %>%
  mutate(price = c(1:1:dim(newTest)))
...
pred.interval <- function(model, percentile) {
    len <- length(newTest[,1])
    preds <- predict(model, newdata=newTest)
    err_down <- array(1:3, c(len,1))
    err_up <- array(1:3, c(len,1))
    for (i in 1:len){
    err_down[i] <- quantile(preds, c((100 - percentile) / 200) )
    err_up[i] <- quantile(preds, c(1 - (100 - percentile) / 200))
    
    }
    error <- array(1:3, c(len,2))
    error[,1] <- err_down
    error[,2] <- err_up
    return(error)
}
interval <- pred.interval(m,95)
```
