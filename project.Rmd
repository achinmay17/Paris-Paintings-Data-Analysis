...
newTrain <- train[,!names(train) %in% c("sale","lot","position","logprice","subject",
                                        ...
                                        "winningbidder","winningbiddertype","endbuyer",
                                        "Interm","type_intermed","Height_in","Width_in","Surface_Rect",
                                        "Diam_in","Surface_Rnd","material","mat","MaterialCat",
                                        "lands_sc","lands_elem","lands_figs","lands_ment")]

#put price as the first column
newTrain <- newTrain[,c(7,1:6,8:35)]
...
m <- lm(formula = log(price) ~ dealer + year + school_pntg + diff_origin + 
    artistliving + Surface + nfigures + engraved + prevcoll + 
    othartist + paired + finished + lrgfont + othgenre + discauth, 
    data = newTrain)

anova(m)

m <- lm(formula = log(price) ~ dealer + year + diff_origin + 
          nfigures + engraved + prevcoll + 
          paired + finished + lrgfont + othgenre + discauth, 
          data = newTrain)
```
...
newTest <- test[,!names(test) %in% c("sale","lot","position","logprice","subject",
                                        ...
                                        "Interm","type_intermed","Height_in","Width_in","Surface_Rect",
                                        "Diam_in","Surface_Rnd","material","mat","MaterialCat",
                                        "lands_sc","lands_elem","lands_figs","lands_ment","price")]
