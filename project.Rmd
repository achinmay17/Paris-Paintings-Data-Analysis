  ...
  mutate(shape_recode = ifelse(Shape == "", "Not Available",
                               ifelse(Shape == "ovale", "oval",
                                      ifelse(Shape == "ronde", "round",
                                             ifelse(Shape == "octogon", "round", Shape)))))
...
newTrain <- newTrain[,c(7,1:6,8:39)]
...
#summary(train$price)
#train$authorstandard[train$price > 10000]
```
...
m1 <- lm(log(price) ~ . + lrgfont:winningbiddertype + 
           ...
           year:diff_origin + year:Surface + diff_origin:Surface +lrgfont:landsALL +lrgfont:finished,dat=newTrain)
#summary(m1)
#plot(m1,ask=FALSE)
...
m <- lm(formula = log(price) ~ dealer + year + school_pntg + diff_origin + 
     ...
     dealer + landsALL:paired + dealer:prevcoll + 
      winningbiddertype:prevcoll + winningbiddertype:artistliving+lrgfont:finished +lrgfont:landsALL, data = newTrain)
...
#        artistliving + winningbiddertype + Surface + nfigures + engraved + 
#        prevcoll + paired + finished + lrgfont + othgenre + discauth + 
#dealer:year+lrgfont:finished +lrgfont:landsALL, data = newTrain)
...
newTest <- test[,!names(test) %in% c("sale","lot","position","logprice","subject",
                                        "authorstyle","authorstandard","author",                          "winningbidder","Interm","Height_in","Width_in","Surface_Rect","Diam_in","Surface_Rnd","material","mat",                            "lands_sc","lands_elem","lands_figs","lands_ment")]
