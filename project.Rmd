...
m= randomForest(log(price)~dealer + year + school_pntg + diff_origin + 
  ...
  #to be confirmed at 6:00 0.997 coverage 1503
```
...
interval <- pred.interval(m,0.977)
```


```{r}
prediction_rf = as.data.frame(interval)
colnames(prediction_rf) <- c("fit","lwr","upr")
save(prediction_rf, file="predict-test.Rdata")
```
...
#cbind(prediction,interval[,1],interval[,2])
#colnames(prediction) <- c("fit")#,"lwr","ubr")
#save(prediction, file="predict-test.Rdata")
