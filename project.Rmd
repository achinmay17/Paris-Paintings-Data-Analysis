...
ln <- lm(formula = log(price) ~ dealer + year + school_pntg + diff_origin + 
        ...
        discauth + winningbiddertype:prevcoll + dealer_author + endbuyer_history +                                   endbuyer_paired +
        Surface:mat_recode + famous_author:Surface, data = newTrain)

#1490 92 without dealer author endbuyer history
#library(randomForest)
#m= randomForest(log(price)~dealer + year + school_pntg + diff_origin + 
#        artistliving + winningbiddertype + Surface + engraved + 
#        prevcoll + paired + finished + lrgfont + othgenre +  
#        discauth + dealer_author + endbuyer_history +                                   endbuyer_paired +
#        Surface:mat_recode + famous_author:Surface, data = newTrain,mtry=5, ntree=15000) #1467
...
#        prevcoll + paired + finished + lrgfont + othgenre +  
#        discauth + dealer_author + endbuyer_history +                                   endbuyer_paired +
#        Surface:mat_recode + famous_author:Surface, data = newTrain,mtry=5, ntree=20000) #1467 92 291 

#m= randomForest(log(price)~dealer + year + school_pntg + diff_origin + 
#        artistliving + winningbiddertype + Surface + engraved + 
#        prevcoll + paired + finished + lrgfont + othgenre +  
#        discauth + dealer_author + endbuyer_history +                                   endbuyer_paired +
#        Surface:mat_recode + famous_author:Surface, data = newTrain,mtry=3, ntree=15000) #1492

m= randomForest(log(price)~ dealer + year + school_pntg + diff_origin + 
        artistliving + winningbiddertype + Surface + engraved + 
        prevcoll + paired + finished + lrgfont + othgenre +  
        discauth + dealer_author + endbuyer_history +                                   endbuyer_paired + mat_recode + famous_author:Surface, data = newTrain,mtry=7, ntree=15000) #deleting winning bidder type and matrecode interaction
...
pred.interval <- function(model, percentile) {
    ...
    preds <- predict(model, newdata=newTest,predict.all=TRUE)
...
interval <- pred.interval(m,0.975)
