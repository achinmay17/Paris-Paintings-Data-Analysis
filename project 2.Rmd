...
suppressWarnings(library(elasticnet))
...
#my.train <- train(log(price) ~ dealer + year + school_pntg + diff_origin + 
#        artistliving + winningbiddertype + Surface + engraved + 
#        prevcoll + paired + finished + lrgfont + othgenre +  
#        discauth + winningbiddertype:prevcoll + 
#        Surface:mat_recode + famous_author:Surface, data = newTrain,
...
#plot(my.train)
#my.train$bestTune
#m <- my.train$finalModel
#my.train$bestTune$lambda
...
#        discauth + winningbiddertype:prevcoll + 
#        Surface:mat_recode + famous_author:Surface, data = newTrain,
#        distribution="gaussian",n.trees=20000,interaction.depth=2)
...
for (i in 1:B){
  ...
  mod_temp=train(fml,tuneGrid=some_model$bestTune,data=newTrain,
                 trControl=tr_control,method="glmnet")
  preds[i,]=exp(predict(mod_temp,newTest))
  if (i %% 100 ==0){
    print(i)
  }
}
mean_b=apply(preds,2,mean)
sd_b=apply(preds,2,sd)
lower_b=mean_b-qnorm(0.975)*sd_b
upper_b=mean_b+qnorm(0.975)*sd_b
Yhat=cbind(mean_b,lower_b,upper_b)
...
predictions = as.data.frame(
  exp(predict(m, newdata=newTest, 
              interval = "predict",type="link",n.trees=20000)))
