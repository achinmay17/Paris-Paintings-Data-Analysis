  ...
  mutate(shape_recode = ifelse(Shape == "", "Not Available",
                               ifelse(Shape == "ovale", "oval",
                                      ifelse(Shape == "ronde", "round",
                                             ifelse(Shape == "octogon", "octagon", Shape)))))
#imputation of Surface
temptest <- mice(test,m=5, maxit=100 ,meth='pmm',seed=500)
completeData <- complete(temptest, 1)
test$Surface <- completeData$Surface
